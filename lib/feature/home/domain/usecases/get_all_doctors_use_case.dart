...
class GetAllDoctorsUseCase
    extends UseCase<ApiResult<List<DoctorModel>>, NoParams> {
  final HomeRepoImpl _homeRepo;
  final FavoriteDoctorService _favoriteService;

  GetAllDoctorsUseCase(this._homeRepo, this._favoriteService);

  @override
  Future<ApiResult<List<DoctorModel>>> call(NoParams params) async {
    final result = await _homeRepo.getAllDoctors();

    return result.when(
      onSuccess: (DoctorsResponse response) {
        final doctors = response.data ?? <DoctorModel>[];
